...
export function BattleArena({ currentUser, allCharacters, shopItems, isAdmin }: BattleArenaProps) {
  ...
  const [editingMinionSkillId, setEditingMinionSkillId] = useState('');
  ...
  const [botSummonSkillEffectDuration, setBotSummonSkillEffectDuration] = useState('1');
  ...
  const [botSummonOwnerSkillId, setBotSummonOwnerSkillId] = useState('');
  ...
  const [botForm, setBotForm] = useState({ name: '', description: '', hp: '30', strength: '9', durability: '6', agility: '5', magic: '0', isBoss: false, avatarUrl: '/avatars/system.svg', avatarFileName: '', encounterChancePercent: '10', passiveTraits: { statusImmunity: false, statusImmunityAll: false, statusImmunityStatuses: [] as string[], healingReceivedMultiplier: '1', copyImmunity: false, reflectImmunity: false, damageBlockedWhileSummonsAlive: false, cleanseAbnormalStatuses: [] as string[], cleanseAllAbnormalStatuses: false, cleanseAbnormalEveryTurns: '5' }, skills: [] as BattleBotSkill[], drops: [] as BattleBotDrop[] });
  ...
  const addBotSkill = () => {
    ...
    if (botSkillEffect === 'summon') {
      if (botSummonSkillMode === 'json') {
        ...
        minionSkills = [{
          ...
          battleEffectDuration:Math.max(1,Math.min(99,Math.floor(Number(botSummonSkillEffectDuration)||1))),
    ...
    const newSkillId = 'bot-skill-' + Date.now();
    const skill: BattleBotSkill = {
      id:newSkillId, name, level:1, multiplier:1, type:'monster',
    ...
    const savedSkillId = botSkillDraftId || newSkillId;
    const skillToSave: BattleBotSkill = botSkillDraftId
      ? { ...skill, id: botSkillDraftId, ...(botSkillEffect === 'summon' ? { summonUnits: botSummonUnits.map(x => ({ ...x, skills: (x.skills || []).map(y => ({ ...y })) })) } : {}) }
      : skill;
    setBotForm(prev => ({ ...prev, skills: botSkillDraftId ? prev.skills.map(existing => getSkillId(existing) === botSkillDraftId ? skillToSave : existing) : [...prev.skills, skillToSave] }));
    if (botSkillEffect === 'summon') setBotSummonOwnerSkillId(savedSkillId);
    ...
    setBotSummonSkillChance('100'); setBotSummonSkillCooldown('0'); setBotSummonSkillEffectDuration('1'); setBotSummonSkillEffect('damage'); setBotSummonSkillPowerMode('flat'); setBotSummonSkillMode('normal');
  ...
  const saveMinionDraftSkill = () => {
    ...
    const skill: BattleBotSkill = {
      id: editingMinionSkillId || "minion-skill-" + Date.now(),
      name: minionDraft.skillName.trim(),
      level: 1,
      multiplier: 1,
      type: "monster-minion",
      ...
      damageScaling: "fixed",
      battleEffect: minionDraft.skillEffect,
      skillCategory: minionDraft.skillCategory,
      targetMode: minionDraft.skillTargetMode,
      targetConfig: minionDraft.skillTargetConfig,
      skillModifiers: minionDraft.skillModifiers.length ? [...minionDraft.skillModifiers] : undefined,
    };
    setMinionDraft(prev => ({
      ...prev,
      skills: editingMinionSkillId
        ? prev.skills.map(existing => existing.id === editingMinionSkillId ? skill : existing)
        : [...prev.skills, skill],
      skillName: "",
      skillDescription: "",
      skillPower: "5",
  ...
  const editMinionDraftSkill = (skill: BattleBotSkill) => {
    setEditingMinionSkillId(skill.id);
    setMinionDraft(prev => ({
      ...prev,
      skillName: skill.name || "",
      skillDescription: skill.description || "",
      skillPower: String(skill.battlePower ?? 5),
      skillPowerMode: skill.battlePowerMode || "flat",
      skillEffectDuration: String(Math.max(1, Number(skill.battleEffectDuration) || 1)),
      skillChance: String(Math.max(0, Math.min(100, Number(skill.aiChancePercent ?? 100) || 0))),
      skillCooldown: String(Math.max(0, Number(skill.cooldownTurns) || 0)),
      skillEffect: skill.battleEffect || "damage",
      skillCategory: skill.skillCategory || "attack",
      skillTargetMode: skill.targetMode || "enemy",
      skillTargetConfig: skill.targetConfig,
      skillModifiers: Array.isArray(skill.skillModifiers) ? skill.skillModifiers.map(item => ({ ...item })) : [],
    }));
  };
  ...
  const addConfiguredMinion = async () => {
    ...
    setMinionDraft({name:'',hp:'20',strength:'5',durability:'1',agility:'1',magic:'0',avatarUrl:'',avatarFileName:'',skills:[],skillName:'',skillDescription:'',skillPower:'5',skillPowerMode:'flat',skillEffectDuration:'1',skillChance:'100',skillCooldown:'0',skillEffect:'damage',skillCategory:'attack',skillTargetMode:'enemy',skillTargetConfig:undefined,skillModifiers:[]}); setEditingMinionId(''); setEditingMinionSkillId(''); setMinionSkillUseLimit('unlimited');
  };
  ...
  const createBot = async (event: React.FormEvent) => {
    ...
    try {
      const persistedSkills = (botForm.skills || []).map(skill => {
        const isOwner = botSummonOwnerSkillId
          ? getSkillId(skill) === botSummonOwnerSkillId
          : skill.battleEffect === 'summon';
        return isOwner && skill.battleEffect === 'summon'
          ? { ...skill, summonUnits: botSummonUnits.map(x => ({ ...x, skills: (x.skills || []).map(y => ({ ...y })) })) }
          : skill;
      });
      await saveBattleBot({ id: editingBotId || 'bot-' + now, name: botForm.name.trim(), description: botForm.description.trim() || 'นักสู้ที่ถูกสร้างโดยแอดมิน', avatarUrl: botForm.avatarUrl || '/avatars/system.svg', avatarFileName: botForm.avatarFileName || undefined, isBoss: botForm.isBoss, stats: { strength: Math.max(0, Number(botForm.strength) || 0), durability: Math.max(0, Number(botForm.durability) || 0), agility: Math.max(0, Number(botForm.agility) || 0), magic: Math.max(0, Number(botForm.magic) || 0) }, hp, maxHp: hp, aiProfile: botForm.isBoss ? 'aggressive' : 'balanced', encounterChancePercent: Math.max(0, Math.min(100, Number(botForm.encounterChancePercent) || 0)),
        passiveTraits: {
          statusImmunity: Boolean(botForm.passiveTraits?.statusImmunity || botForm.passiveTraits?.statusImmunityAll),
          statusImmunityAll: Boolean(botForm.passiveTraits?.statusImmunityAll),
          statusImmunityStatuses: Array.isArray(botForm.passiveTraits?.statusImmunityStatuses) ? [...botForm.passiveTraits.statusImmunityStatuses] : [],
          healingReceivedMultiplier: Math.max(0, Math.min(10, Number(botForm.passiveTraits?.healingReceivedMultiplier) || 1)),
          copyImmunity: Boolean(botForm.passiveTraits?.copyImmunity),
          reflectImmunity: Boolean(botForm.passiveTraits?.reflectImmunity),
          damageBlockedWhileSummonsAlive: Boolean(botForm.passiveTraits?.damageBlockedWhileSummonsAlive),
          cleanseAbnormalStatuses: Array.isArray(botForm.passiveTraits?.cleanseAbnormalStatuses) ? [...botForm.passiveTraits.cleanseAbnormalStatuses] : [],
          cleanseAllAbnormalStatuses: Boolean(botForm.passiveTraits?.cleanseAllAbnormalStatuses),
          cleanseAbnormalEveryTurns: Math.max(1, Math.min(99, Number(botForm.passiveTraits?.cleanseAbnormalEveryTurns) || 5)),
        },
        skills: persistedSkills.map(skill => ({ ...skill })), drops: (botForm.drops || []).map(drop => ({ ...drop, itemData: drop.itemData ? { ...drop.itemData } : undefined })), createdAt: now, updatedAt: now });
      setBotSummonOwnerSkillId(''); setBotSummonUnits([]); setBotForm({ name: '', description: '', hp: '30', strength: '9', durability: '6', agility: '5', magic: '0', isBoss: false, avatarUrl: '/avatars/system.svg', avatarFileName: '', encounterChancePercent: '10', passiveTraits: { statusImmunity: false, statusImmunityAll: false, statusImmunityStatuses: [], healingReceivedMultiplier: '1', copyImmunity: false, reflectImmunity: false, damageBlockedWhileSummonsAlive: false, cleanseAbnormalStatuses: [], cleanseAllAbnormalStatuses: false, cleanseAbnormalEveryTurns: '5' }, skills: [], drops: [] }); setEditingBotId('');
...
<div className="text-[10px] text-slate-500">ระบบใช้เฉพาะ Reward Pool ของโหมดนี้ — Coins แสดงเป็นจำนวน Coins, Item แสดงชื่อ Item, Skill แสดงเฉพาะรายการที่ Admin เพิ่มเป็น Skill เท่านั้น</div>
  ...
  <div className="mb-1 text-sm font-black text-amber-200">🛡️ Passive พิเศษของมอน / บอส</div>
  <p className="mb-3 text-[10px] leading-relaxed text-slate-400">ตั้งกติกาเฉพาะตัวได้ เช่น กันสถานะ, รับฮีล 2 เท่า, กันการ Copy และกันความเสียหายสะท้อน</p>
  <div className="grid gap-2 sm:grid-cols-2">
    <label className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/50 p-3 text-xs text-slate-200">
      <input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunity)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunity:e.target.checked}}))} />
      🚫 ไม่รับสถานะผิดปกติ
    </label>
    <label className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/50 p-3 text-xs text-slate-200">
      <input type="checkbox" checked={Boolean(botForm.passiveTraits?.copyImmunity)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),copyImmunity:e.target.checked}}))} />
      🧬 ไม่สามารถถูก Copy ความสามารถ
    </label>
    <label className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/50 p-3 text-xs text-slate-200">
      <input type="checkbox" checked={Boolean(botForm.passiveTraits?.reflectImmunity)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),reflectImmunity:e.target.checked}}))} />
      🪞 ต้านความเสียหายสะท้อน
    </label>
    <label className="text-[10px] text-slate-500">ตัวคูณฟื้นฟู HP ที่ได้รับ (เช่น 2 = 2 เท่า)
      <input className={inputClass+" mt-1"} type="number" min="0" max="10" step="0.1" value={botForm.passiveTraits?.healingReceivedMultiplier ?? '1'} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),healingReceivedMultiplier:e.target.value}}))} />
    </label>
    <label className="flex items-center gap-2 rounded-xl border border-amber-400/20 bg-amber-950/20 p-3 text-xs text-amber-100 sm:col-span-2">
      <input type="checkbox" checked={Boolean(botForm.passiveTraits?.damageBlockedWhileSummonsAlive)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),damageBlockedWhileSummonsAlive:e.target.checked}}))} />
      🧿 ลูกน้องยังไม่ตาย = มอน/บอสตัวนี้ไม่รับดาเมจ
    </label>
    <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3 sm:col-span-2">
      <div className="mb-2 text-xs font-black text-slate-200">🚫 เลือกสถานะที่มอน/บอสไม่รับ</div>
      <label className="flex items-center gap-2 text-xs text-slate-300">
        <input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityAll)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityAll:e.target.checked,statusImmunity:e.target.checked}}))} />
        ไม่รับสถานะผิดปกติทั้งหมด
      </label>
      <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-4">
        <label key="stun" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("stun"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"stun"])]:current.filter(x=>x!=="stun"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>💫 Stun</label><label key="freeze" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("freeze"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"freeze"])]:current.filter(x=>x!=="freeze"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>❄️ Freeze</label><label key="poison" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("poison"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"poison"])]:current.filter(x=>x!=="poison"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>☠️ Poison</label><label key="burn" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("burn"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"burn"])]:current.filter(x=>x!=="burn"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>🔥 Burn</label><label key="bleeding" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("bleeding"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"bleeding"])]:current.filter(x=>x!=="bleeding"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>🩸 Bleeding</label><label key="slow" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("slow"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"slow"])]:current.filter(x=>x!=="slow"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>🐌 Slow</label><label key="curse" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("curse"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"curse"])]:current.filter(x=>x!=="curse"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>🌀 Curse</label><label key="weakness" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("weakness"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"weakness"])]:current.filter(x=>x!=="weakness"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>📉 Weakness</label><label key="damage_taken_increase" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.statusImmunityStatuses?.includes("damage_taken_increase"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.statusImmunityStatuses||[]; const next=e.target.checked?[...new Set([...current,"damage_taken_increase"])]:current.filter(x=>x!=="damage_taken_increase"); return {...p,passiveTraits:{...(p.passiveTraits||{}),statusImmunityStatuses:next}}})}/>💥 รับดาเมจมากขึ้น</label>
      </div>
    </div>
    <div className="rounded-xl border border-cyan-400/20 bg-cyan-950/20 p-3 sm:col-span-2">
      <div className="mb-2 text-xs font-black text-cyan-100">🧼 Passive ลบสถานะผิดปกติ</div>
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="flex items-center gap-2 text-xs text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAllAbnormalStatuses)} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAllAbnormalStatuses:e.target.checked}}))}/> ลบสถานะผิดปกติทั้งหมด</label>
        <label className="text-[10px] text-slate-500">ทุกกี่เทิร์น
          <input className={inputClass+" mt-1"} type="number" min="1" max="99" value={botForm.passiveTraits?.cleanseAbnormalEveryTurns ?? '5'} onChange={e=>setBotForm(p=>({...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalEveryTurns:e.target.value}}))}/>
        </label>
      </div>
      <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-4">
        <label key="clean-stun" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("stun"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"stun"])]:current.filter(x=>x!=="stun"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>💫 Stun</label><label key="clean-freeze" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("freeze"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"freeze"])]:current.filter(x=>x!=="freeze"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>❄️ Freeze</label><label key="clean-poison" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("poison"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"poison"])]:current.filter(x=>x!=="poison"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>☠️ Poison</label><label key="clean-burn" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("burn"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"burn"])]:current.filter(x=>x!=="burn"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>🔥 Burn</label><label key="clean-bleeding" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("bleeding"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"bleeding"])]:current.filter(x=>x!=="bleeding"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>🩸 Bleeding</label><label key="clean-slow" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("slow"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"slow"])]:current.filter(x=>x!=="slow"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>🐌 Slow</label><label key="clean-curse" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("curse"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"curse"])]:current.filter(x=>x!=="curse"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>🌀 Curse</label><label key="clean-weakness" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("weakness"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"weakness"])]:current.filter(x=>x!=="weakness"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>📉 Weakness</label><label key="clean-damage_taken_increase" className="flex items-center gap-1 rounded bg-slate-900 px-2 py-1 text-[10px] text-slate-300"><input type="checkbox" checked={Boolean(botForm.passiveTraits?.cleanseAbnormalStatuses?.includes("damage_taken_increase"))} onChange={e=>setBotForm(p=>{const current=p.passiveTraits?.cleanseAbnormalStatuses||[]; const next=e.target.checked?[...new Set([...current,"damage_taken_increase"])]:current.filter(x=>x!=="damage_taken_increase"); return {...p,passiveTraits:{...(p.passiveTraits||{}),cleanseAbnormalStatuses:next}}})}/>💥 รับดาเมจมากขึ้น</label>
      </div>
      <div className="mt-1 text-[9px] text-slate-500">ถ้าไม่เลือก “ทั้งหมด” จะลบเฉพาะสถานะที่ติ๊กไว้ เช่น ทุก 5 เทิร์นลบ Poison + Burn</div>
    </div>
  </div>
</div><div className="grid gap-2 sm:grid-cols-2"><label className="text-[10px] text-slate-500">โอกาสถูกสุ่มเจอ (%)<input className={inputClass + ' mt-1'} type="number" min="0" max="100" value={botForm.encounterChancePercent} onChange={event => setBotForm(prev => ({ ...prev, encounterChancePercent: event.target.value }))} /></label><div className="text-[10px] text-slate-500 rounded-xl border border-amber-500/20 bg-amber-950/10 p-2">โหมดสุ่มจะใช้ค่านี้เป็นน้ำหนักการออกของมอน/บอส</div></div>
...
<div className="rounded-2xl border border-fuchsia-500/20 bg-fuchsia-950/10 p-3">
  ...
  <div className="grid gap-2 sm:grid-cols-2">
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      ...
      {['defense','heal','damage_reduction','damage_taken_increase','reflect','reflect_no_damage'].includes(botSkillEffect) && <div className="sm:col-span-2 rounded-xl border border-cyan-400/30 bg-cyan-950/10 p-2">
...
{['reflect','reflect_no_damage','damage_reduction','defense','heal'].includes(minionDraft.skillEffect) && <label className="text-[11px] text-slate-400">⏱️ ระยะเวลาผล (เทิร์น)
...
<SkillBattleOptions config={{skillCategory:minionDraft.skillCategory,targetMode:minionDraft.skillTargetMode,targetConfig:minionDraft.skillTargetConfig,skillModifiers:minionDraft.skillModifiers,battleEffect:minionDraft.skillEffect}} onChange={patch=>setMinionDraft(prev=>({...prev,skillCategory:patch.skillCategory ?? prev.skillCategory,skillTargetMode:patch.targetMode ?? prev.skillTargetMode,skillTargetConfig:patch.targetConfig ?? prev.skillTargetConfig,skillModifiers:patch.skillModifiers ?? prev.skillModifiers,skillEffect:patch.battleEffect ?? prev.skillEffect}))} /><label className="text-[11px] text-slate-400">จำนวนครั้งต่อเกม<select className={inputClass+" mt-1"} value={minionSkillUseLimit} onChange={e=>setMinionSkillUseLimit(e.target.value as 'unlimited' | 'once_per_battle')}><option value="unlimited">ใช้ได้หลายครั้งตามคูลดาวน์</option><option value="once_per_battle">ใช้ได้ 1 ครั้งต่อเกม</option></select></label>
      ...
      <div className="flex gap-2 mt-2">
        ...
        {editingMinionId && <button type="button" className={buttonClass+" bg-slate-700 text-white"} onClick={()=>{setEditingMinionId('');setMinionDraft({name:'',hp:'20',strength:'5',durability:'1',agility:'1',magic:'0',avatarUrl:'',avatarFileName:'',skills:[],skillName:'',skillDescription:'',skillPower:'5',skillPowerMode:'flat',skillEffectDuration:'1',skillChance:'100',skillCooldown:'0',skillEffect:'damage',skillCategory:'attack',skillTargetMode:'enemy',skillTargetConfig:undefined,skillModifiers:[]});setMinionSkillUseLimit('unlimited');}}>ยกเลิก</button>}
      </div>
      {editingMinionId && <div className="mt-2 rounded-lg border border-amber-400/30 bg-amber-950/20 p-2 text-[10px] text-amber-200">💡 บันทึกตรงนี้จะอัปเดตลูกน้องในรายการทันที และกด “บันทึกการแก้ไขบอท” ด้านล่างอีกครั้งเพื่อบันทึกเข้าข้อมูลบอท</div>}
...
setBotSummonName(skill.summonName||'ลูกน้อง');setBotSummonMaxCount(String(skill.summonMaxCount??1));setBotSummonHp(String(skill.summonHp??20));setBotSummonDamage(String(skill.summonDamage??5));setBotSummonAgility(String(skill.summonAgility??1));setBotSummonAvatarUrl(skill.summonAvatarUrl||'');setBotSummonAvatarFileName(skill.summonAvatarFileName||'');setBotSummonSkillsText(JSON.stringify(skill.summonSkills||[],null,2));setBotSummonOwnerSkillId(getSkillId(skill));setBotSummonUnits((skill.summonUnits||[]).map(x=>({...x,skills:(x.skills||[]).map(y=>({...y}))})));}}>✏️ แก้ไข</button><button type="button" className="mt-1 text-rose-300" onClick={()=>removeBotSkill(getSkillId(skill))}>ลบ</button></div>)}</div>
<button type="submit" className={buttonClass + ' w-full bg-rose-500 text-white hover:bg-rose-400'}><Plus className="mr-1 inline h-4 w-4" />{editingBotId ? "บันทึกการแก้ไขบอท" : "สร้างบอท"}</button></form></div>{bots.length > 0 && <div className="mt-6 grid gap-3 md:grid-cols-2">{bots.map(bot => <div key={bot.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/50 p-3"><div className="flex min-w-0 items-center gap-3"><img src={bot.avatarUrl} alt="" className="h-10 w-10 rounded-xl border border-slate-700 object-cover" /><div className="min-w-0"><div className="flex items-center gap-2 truncate font-bold text-white">{bot.isBoss && <Crown className="h-3.5 w-3.5 text-amber-300" />}{bot.name}</div><div className="text-[11px] text-slate-500">HP {bot.maxHp} · พลัง {bot.stats.strength} · {bot.description}</div></div></div><button type="button" onClick={() => { setEditingBotId(bot.id); const restoredSummonSkill = (bot.skills || []).find(skill => skill.battleEffect === 'summon' && Array.isArray(skill.summonUnits)); setBotSummonOwnerSkillId(restoredSummonSkill?.id || ''); setBotSummonUnits((restoredSummonSkill?.summonUnits || []).map(x => ({ ...x, skills: (x.skills || []).map(y => ({ ...y })) }))); setBotForm({ name: bot.name, description: bot.description, hp: String(bot.maxHp), strength: String(bot.stats.strength), durability: String(bot.stats.durability), agility: String(bot.stats.agility), magic: String(bot.stats.magic), isBoss: bot.isBoss, avatarUrl: bot.avatarUrl, avatarFileName: bot.avatarFileName || '', encounterChancePercent: String(bot.encounterChancePercent ?? 10), passiveTraits: {
            statusImmunity: Boolean(bot.passiveTraits?.statusImmunity),
            statusImmunityAll: Boolean(bot.passiveTraits?.statusImmunityAll ?? bot.passiveTraits?.statusImmunity),
            statusImmunityStatuses: Array.isArray(bot.passiveTraits?.statusImmunityStatuses) ? [...bot.passiveTraits.statusImmunityStatuses] : [],
            healingReceivedMultiplier: String(bot.passiveTraits?.healingReceivedMultiplier ?? 1),
            copyImmunity: Boolean(bot.passiveTraits?.copyImmunity),
            reflectImmunity: Boolean(bot.passiveTraits?.reflectImmunity),
            damageBlockedWhileSummonsAlive: Boolean(bot.passiveTraits?.damageBlockedWhileSummonsAlive),
            cleanseAbnormalStatuses: Array.isArray(bot.passiveTraits?.cleanseAbnormalStatuses) ? [...bot.passiveTraits.cleanseAbnormalStatuses] : [],
            cleanseAllAbnormalStatuses: Boolean(bot.passiveTraits?.cleanseAllAbnormalStatuses),
            cleanseAbnormalEveryTurns: String(bot.passiveTraits?.cleanseAbnormalEveryTurns ?? 5)
          }, skills: (bot.skills || []).map(skill => ({ ...skill })), drops: (bot.drops || []).map(drop => ({ ...drop, itemData: drop.itemData ? { ...drop.itemData } : undefined })) }); setBotDropChance('100'); }} className="rounded-lg p-2 text-sky-300 hover:bg-sky-500/15">✏️</button><button type="button" onClick={() => void deleteBattleBot(bot.id)} className="rounded-lg p-2 text-slate-500 hover:bg-rose-500/15 hover:text-rose-300"><Trash2 className="h-4 w-4" /></button></div>)}</div>}</section>}
