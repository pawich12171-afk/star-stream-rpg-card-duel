...
{minionDraft.skills.length>0&&<div className="mt-2 space-y-1">{minionDraft.skills.map((s,i)=><div key={s.id} className="rounded-lg border border-slate-700 px-2 py-1 text-[11px]"><b>{i+1}. {s.name}</b> — {s.description} · พลัง {s.battlePower} · {s.battleEffect}<button type="button" className="ml-2 text-rose-300" onClick={()=>setMinionDraft({...minionDraft,skills:minionDraft.skills.filter(x=>x.id!==s.id)})}>ลบ</button></div>)}</div>}
      ...
      <div className="flex gap-2 mt-2">
        ...
        {editingMinionId && <button type="button" className={buttonClass+" bg-slate-700 text-white"} onClick={()=>{setEditingMinionId('');setMinionDraft({name:'',hp:'20',strength:'5',durability:'1',agility:'1',magic:'0',avatarUrl:'',avatarFileName:'',skills:[],skillName:'',skillDescription:'',skillPower:'5',skillPowerMode:'flat',skillEffectDuration:'1',skillChance:'100',skillCooldown:'0',skillEffect:'damage'});setMinionSkillUseLimit('unlimited');}}>ยกเลิก</button>}
      </div>
