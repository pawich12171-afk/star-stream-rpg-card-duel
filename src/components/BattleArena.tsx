...
function makeDiceFaces(existing: BattleDiceFace[], sides: number): BattleDiceFace[] {
...
function effectStyle(effect?: BattleDiceFace['effect'] | 'stun_skip'): string {
...
export function BattleArena({ currentUser, allCharacters, isAdmin }: BattleArenaProps) {
  ...
  const diceFaces = useMemo(() => makeDiceFaces(config.faces || [], config.sides), [config.faces, config.sides]);
  ...
  const persistBattleHp = async (room: BattleRoom) => {
    const playerUnits = [...room.teamA, ...room.teamB].filter(unit => unit.type === 'player');
    await Promise.all(playerUnits.map(async unit => {
      const character = allCharacters.find(item => item.id === unit.sourceId);
      if (!character || character.hp === unit.hp) return;
      await updateCharacterInDB({ ...character, hp: Math.max(0, Math.min(character.maxHp, unit.hp)), lastUpdated: Date.now() });
    }));
  };
  ...
  const updateFace = (faceNumber: number, patch: Partial<BattleDiceFace>) => {
    setConfig(prev => ({ ...prev, faces: makeDiceFaces(prev.faces || [], prev.sides).map(face => face.face === faceNumber ? { ...face, ...patch } : face) }));
  };

  const setDiceSides = (nextSides: number) => {
    const sides = Math.max(2, Math.min(100, Math.round(nextSides) || 6));
    setConfig(prev => ({ ...prev, sides, faces: makeDiceFaces(prev.faces || [], sides) }));
  };
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-cyan-500/30 bg-gradient-to-br from-cyan-950/60 via-slate-900 to-violet-950/40 p-5 md:p-7 shadow-2xl">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-cyan-400/20 bg-slate-950/40 p-3"><div className="text-[10px] uppercase tracking-widest text-slate-500">ลูกเต๋า</div><div className="mt-1 font-black text-violet-200">D{config.sides} / {diceFaces.length} ผลลัพธ์</div></div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-violet-500/20 bg-violet-950/10 p-3">
                  <label className="text-xs text-slate-400">จำนวนหน้าเต๋า (สูงสุด 100)<input className={inputClass + ' mt-1 w-32'} type="number" min="2" max="100" value={config.sides} onChange={event => setDiceSides(Number(event.target.value))} /></label>
                  <button type="button" onClick={() => setDiceSides(config.sides + 1)} className={buttonClass + ' bg-violet-500 text-white hover:bg-violet-400'}><Plus className="mr-1 inline h-3.5 w-3.5" />เพิ่มหน้า</button>
                  <button type="button" onClick={() => setDiceSides(config.sides - 1)} disabled={config.sides <= 2} className={buttonClass + ' bg-slate-800 text-slate-300 hover:bg-slate-700 disabled:opacity-40'}>ลดหน้าสุดท้าย</button>
                  <span className="text-[11px] text-violet-200">ตอนนี้ใช้ D{config.sides} — ปรับเป็น D8, D12, D20 หรือมากกว่านั้นได้</span>
                </div>
                {diceFaces.map(face => <div key={face.face} className="grid gap-2 rounded-2xl border border-slate-800 bg-slate-950/50 p-3 sm:grid-cols-[2.5rem_7rem_5rem_1fr] sm:items-center"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-violet-500/20 font-black text-violet-200">{face.face}</div><select className={inputClass} value={face.effect} onChange={event => updateFace(face.face, { effect: event.target.value as BattleDiceFace['effect'] })}><option value="damage">โจมตี</option><option value="critical">คริติคอล</option><option value="heal">ฟื้นฟู</option><option value="miss">พลาด</option><option value="stun">สตัน</option></select><input className={inputClass} type="number" step="0.5" value={face.value} onChange={event => updateFace(face.face, { value: Number(event.target.value) || 0 })} placeholder="ค่า" /><div className="grid gap-2 sm:grid-cols-2"><input className={inputClass} value={face.label} onChange={event => updateFace(face.face, { label: event.target.value })} placeholder="ชื่อผล" /><input className={inputClass} value={face.description} onChange={event => updateFace(face.face, { description: event.target.value })} placeholder="คำอธิบาย" /></div></div>)}
              </div>
            </div>
