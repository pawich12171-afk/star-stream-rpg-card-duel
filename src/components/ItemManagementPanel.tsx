      ...
      <div className="rounded-3xl bg-slate-900/90 border border-fuchsia-500/30 shadow-2xl overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
          ...
          <div className="lg:col-span-2 bg-gradient-to-b from-slate-900/90 to-slate-950/80 border border-slate-700/70 rounded-3xl p-5 shadow-2xl shadow-black/20">
            <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4"><div><div className="text-lg font-black text-white">คลังไอเทมทั้งหมด</div><div className="text-[11px] text-slate-500">จัดการไอเทมกลางของเว็บไซต์</div></div><div className="flex-1 flex items-center gap-2 rounded-xl bg-slate-900 border border-slate-700 px-3"><Search className="w-4 h-4 text-slate-500"/><input className="flex-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-sm text-white" placeholder="ค้นหาชื่อหรือ ID..." value={search} onChange={e=>setSearch(e.target.value)}/></div>
            <div className="space-y-2 max-h-[650px] overflow-y-auto pr-1">
              {filtered.map(item => <div key={item.id} className="group rounded-2xl border border-slate-800 bg-slate-900/90 p-4 hover:border-fuchsia-500/30 hover:bg-slate-800/80 transition-all">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="min-w-0 flex items-center gap-3"><div className="w-12 h-12 shrink-0 rounded-xl overflow-hidden border border-slate-700 bg-slate-950 flex items-center justify-center">{item.icon?.startsWith("data:")||item.icon?.startsWith("http")?<img src={item.icon} alt="" className="w-full h-full object-cover"/>:<Package className="w-5 h-5 text-slate-500"/>}</div><div className="min-w-0"><div className="font-black text-white truncate">{item.name}</div><div className="text-[10px] text-slate-500">ID: {item.id}</div><div className="flex flex-wrap gap-1.5 mt-2"><span className="text-[9px] px-2 py-1 rounded-full border border-slate-700 text-slate-300">{item.rarity}</span><span className="text-[9px] px-2 py-1 rounded-full border border-emerald-500/30 text-emerald-300">{item.inShop&&!item.adminOnly?'SHOP':'ไม่ลง SHOP'}</span><span className="text-[9px] px-2 py-1 rounded-full border border-amber-500/30 text-amber-300">{item.rewardEligible===false?'ไม่ใช้เป็นรางวัล':'ใช้เป็นรางวัล'}</span><span className="text-[9px] px-2 py-1 rounded-full border border-fuchsia-500/30 text-fuchsia-300">{item.stackable===false?'ไม่ Stack':'Stack'}</span><span className="text-[9px] px-2 py-1 rounded-full border border-cyan-500/30 text-cyan-300">{item.itemClass==='limited'?'LIMITED':item.itemClass==='special'?'SPECIAL':'NORMAL'}</span></div></div>
