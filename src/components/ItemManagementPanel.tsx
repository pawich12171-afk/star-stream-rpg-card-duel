        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
          <form onSubmit={submit} className="bg-slate-950/70 border border-slate-800 rounded-2xl p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="text-[11px] text-slate-400">ค่าผลหลัก (ถ้าใช้)<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white" type="number" min="0" value={effectValue} onChange={e=>setEffectValue(Number(e.target.value))}/></label>
            </div>
            ...
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 space-y-3">
              <div><div className="text-sm font-black text-emerald-200">✨ เอฟเฟกต์หลักของไอเทม</div><p className="text-[10px] text-slate-500 mt-1">เลือกเอฟเฟกต์หลักและกรอกค่าที่ต้องการ ระบบจะบันทึกค่าเหล่านี้ลงไอเทม</p></div>
              <select className="w-full rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white text-sm" value={effectType} onChange={e=>setEffectType(e.target.value as any)}>
                <option value="heal_hp">❤️ ฟื้น HP</option><option value="boost_max_hp">❤️ เพิ่ม Max HP</option><option value="buff_stat">💪 เพิ่ม STR / DUR / AGI / MAG</option><option value="enhance_skill">⚔️ เสริมสกิล</option><option value="custom">✨ เอฟเฟกต์กำหนดเอง / ใช้รายละเอียดด้านล่าง</option>
              </select>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <label className="text-[10px] text-slate-400">❤️ ฟื้น HP (หน่วย)<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 10" value={effectValue || ''} onChange={e=>setEffectValue(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">❤️ ฟื้น HP (% ของ Max HP)<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white" type="number" min="0" max="100" placeholder="เช่น 20" value={healPercent || ''} onChange={e=>setHealPercent(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">❤️ เพิ่ม Max HP<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 100" value={hpBonus || ''} onChange={e=>setHpBonus(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">💪 ค่าพลัง Stat ที่เพิ่ม<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 10" value={effectValue || ''} onChange={e=>setEffectValue(Number(e.target.value)||0)}/></label>
              </div>
              {effectType === 'buff_stat' && <label className="text-[10px] text-slate-400 block">ประเภท Stat<select className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white text-sm" value={targetStat} onChange={e=>setTargetStat(e.target.value as any)}><option value="strength">STR — ความแข็งแกร่ง</option><option value="durability">DUR — ความทนทาน</option><option value="agility">AGI — ความคล่องตัว</option><option value="magic">MAG — พลังเวท</option></select></label>}
              {effectType === 'enhance_skill' && <div className="space-y-2"><label className="text-[10px] text-slate-400">ชื่อ/ID สกิล<input className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white text-sm" placeholder="เช่น Fire Slash" value={skillTarget} onChange={e=>setSkillTarget(e.target.value)}/></label><label className="text-[10px] text-slate-400">รายละเอียดสิ่งที่เพิ่มให้สกิล<textarea className="w-full mt-1 rounded-xl bg-slate-900 border border-slate-700 p-2.5 text-white text-sm" placeholder="เช่น Damage +25%, ลดคูลดาวน์ 1 เทิร์น" value={skillDesc} onChange={e=>setSkillDesc(e.target.value)}/></label></div>}
              <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-500"><div>🎯 Critical % = โอกาสคริติคอล</div><div>🔁 ตีซ้ำ % = โอกาสโจมตีซ้ำ</div><div>🩸 ดูดเลือด % = ดูด HP จากดาเมจ</div><div>⏳ ลดคูลดาวน์ % = ลดคูลดาวน์สกิล</div></div>
            </div>
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-900/70 p-3 space-y-3">
              <div><div className="text-xs font-black text-white">✨ รายละเอียดเอฟเฟกต์เมื่อใช้ไอเทม</div><p className="text-[10px] text-slate-500 mt-1">กรอกเฉพาะเอฟเฟกต์ที่ต้องการใช้ ค่า 0 หรือปิดสวิตช์ = ไม่เกิดผล</p></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <label className="text-[10px] text-slate-400">❤️ ชุบชีวิต: ฟื้นกี่ % ของ Max HP<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={revivePercent} onChange={e=>setRevivePercent(Number(e.target.value))}/></label>
                <label className="flex items-center gap-2 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-xs text-white mt-5"><input type="checkbox" checked={reviveAlly} onChange={e=>setReviveAlly(e.target.checked)}/> ชุบเพื่อน/สมาชิกทีมได้</label>
                <label className="text-[10px] text-slate-400">🛡️ โล่: ป้องกันกี่ % ของ Max HP<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={shieldPercent} onChange={e=>setShieldPercent(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">⏱️ โล่อยู่กี่เทิร์น<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" value={shieldDuration} onChange={e=>setShieldDuration(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">🛡️ ลดความเสียหายกี่ %<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={damageReductionPercent} onChange={e=>setDamageReductionPercent(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">🍃 โอกาสหลบหลีกกี่ %<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={dodgeChancePercent} onChange={e=>setDodgeChancePercent(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">🩸 ดูดเลือดกี่ %<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={lifestealPercent} onChange={e=>setLifestealPercent(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">⏳ ลดคูลดาวน์กี่ %<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" max="100" value={cooldownReductionPercent} onChange={e=>setCooldownReductionPercent(Number(e.target.value))}/></label>
                <label className="text-[10px] text-slate-400">🚫 ต้านสถานะผิดปกติกี่เทิร์น<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white text-xs" type="number" min="0" value={statusImmunityDuration} onChange={e=>setStatusImmunityDuration(Number(e.target.value))}/></label>
                <label className="sm:col-span-2 flex items-center gap-2 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-xs text-white"><input type="checkbox" checked={cleanseNegative} onChange={e=>setCleanseNegative(e.target.checked)}/> 🧼 ล้างสถานะผิดปกติเมื่อใช้</label>
              </div>
            </div>
            <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-3 space-y-3">
              <div><div className="text-sm font-black text-orange-200">⚔️ เอฟเฟกต์ต่อสู้ / กาชา</div><p className="text-[10px] text-slate-500">แต่ละช่องบอกชัดเจนว่าค่าที่กรอกส่งผลอะไร</p></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <label className="text-[10px] text-slate-400">⚔️ เพิ่ม Damage (%)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 25" value={battleDamagePercent || ''} onChange={e=>setBattleDamagePercent(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">⏱️ Damage อยู่กี่เทิร์น<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 3" value={battleDamageDuration || ''} onChange={e=>setBattleDamageDuration(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">🎯 Critical (%)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" max="100" placeholder="เช่น 20" value={battleCriticalChancePercent || ''} onChange={e=>setBattleCriticalChancePercent(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">🔁 โอกาสตีซ้ำ (%)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" max="100" placeholder="เช่น 10" value={battleRepeatAttackChancePercent || ''} onChange={e=>setBattleRepeatAttackChancePercent(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">🍀 ตัวคูณโชค (เท่า)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 2 = x2" value={battleLuckMultiplier || ''} onChange={e=>setBattleLuckMultiplier(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">⏱️ โชคอยู่กี่เทิร์น<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 3" value={battleLuckDuration || ''} onChange={e=>setBattleLuckDuration(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">✨ ตัวคูณโอกาส Passive / Effect (เท่า)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 2 = x2" value={battlePassiveChanceMultiplier || ''} onChange={e=>setBattlePassiveChanceMultiplier(Number(e.target.value)||0)}/></label>
                <label className="text-[10px] text-slate-400">🎲 ตัวคูณเรทกาชา (เท่า)<input className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" type="number" min="0" placeholder="เช่น 2 = x2" value={gachaRateMultiplier || ''} onChange={e=>setGachaRateMultiplier(Number(e.target.value)||0)}/></label>
              </div>
              {gachaRateMultiplier > 0 && <label className="text-[10px] text-slate-400 block">🎲 โบนัสเรทเริ่มตั้งแต่ระดับ<select className="w-full mt-1 rounded-xl bg-slate-950 border border-slate-700 p-2.5 text-white" value={gachaRateMinRarity} onChange={e=>setGachaRateMinRarity(e.target.value as GachaRarity)}><option value="rare">Rare</option><option value="epic">Epic</option><option value="legendary">Legendary</option><option value="mythic">Mythic</option></select></label>}
            </div>
