...
import { CharacterProfile, Item, InventoryItem, GachaRarity, ItemPassiveEffect, MarketplaceListing, MarketplaceAuction, ItemTrade } from '../types';
import { 
  ShoppingBag, 
  Package, 
  Coins, 
...
interface ShopInventoryProps {
  ...
  onCreateItemTrade?: (recipientId: string, offeredItemInstanceId: string, offeredQuantity: number, offeredCoins: number, requestedItemInstanceId?: string, requestedQuantity?: number, requestedCoins?: number) => Promise<void>;
  onAcceptItemTrade?: (tradeId: string) => Promise<void>;
  onCancelItemTrade?: (tradeId: string) => Promise<void>;
...
export const ShopInventory: React.FC<ShopInventoryProps> = ({
  ...
  const [tradeCoins, setTradeCoins] = useState('0');
  const [tradeRequestedItemId, setTradeRequestedItemId] = useState('');
  const [tradeRequestedQuantity, setTradeRequestedQuantity] = useState('1');
  const [tradeRequestedCoins, setTradeRequestedCoins] = useState('0');
  ...
  const handleUpgradeEquipmentSlot = async () => {
    ...
    if (!progress || Number(progress.magic || 0) < 100) {
      alert('ต้องอัปสกิลตามลำดับจนค่าเวทครบ 100 ก่อน จึงจะปลดล็อกช่องสวมใส่ +2 ช่อง');
      return;
    }
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 max-h-[72vh] overflow-y-auto overflow-x-hidden pr-1 scrollbar-thin">
                  ...
                  >
                    ...
                    <div className="pt-3 border-t border-slate-800 flex flex-col gap-3 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 min-w-0 w-full">
                        ...
                        <button type="button" onClick={() => { setTransferItem(invItem); setTransferTarget(''); setTransferQuantity('1'); }} className="px-3.5 py-1.5 text-xs font-bold rounded-xl bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-300 border border-cyan-500/30 cursor-pointer">🎁 โอน</button><button type="button" onClick={() => { setTradeItem(invItem); setTradeTarget(''); setTradeQuantity('1'); setTradeCoins('0'); setTradeRequestedItemId(''); setTradeRequestedQuantity('1'); setTradeRequestedCoins('0'); }} className="px-3.5 py-1.5 text-xs font-bold rounded-xl bg-fuchsia-600/20 hover:bg-fuchsia-600/30 text-fuchsia-300 border border-fuchsia-500/30 cursor-pointer"><ArrowLeftRight className="w-3.5 h-3.5 inline mr-1" />เทรด</button>
        ...
        <div className="mb-5 rounded-2xl border border-fuchsia-500/30 bg-fuchsia-950/10 p-4">
          ...
          <div className="space-y-2">
            {itemTrades.map(trade => {
              const incoming = trade.recipientId === character.id;
              return <div key={trade.id} className="rounded-xl border border-slate-700 bg-slate-950/60 p-3">
                <div className="text-xs font-black text-white">{incoming ? '📨 ' + trade.senderName + ' เสนอเทรดให้คุณ' : '📤 เสนอเทรดให้ ' + trade.recipientName}</div>
                <div className="text-[11px] text-slate-400 mt-1">{trade.offeredItem?.name || 'ไอเทม'} ×{trade.offeredQuantity}{trade.offeredCoins > 0 ? ' + ' + formatCoins(trade.offeredCoins) + ' Coins' : ''}{' ↔ '}{trade.requestedItem?.name ? trade.requestedItem.name + ' ×' + trade.requestedQuantity : 'ไม่ขอไอเทม'}{trade.requestedCoins > 0 ? ' + ' + formatCoins(trade.requestedCoins) + ' Coins' : ''}</div>
                <div className="flex gap-2 mt-2">
                  {incoming && <button type="button" disabled={tradeBusyId===trade.id} onClick={async()=>{setTradeBusyId(trade.id);try{await onAcceptItemTrade?.(trade.id);alert('เทรดสำเร็จ')}catch(e){alert(e instanceof Error?e.message:'รับเทรดไม่สำเร็จ')}finally{setTradeBusyId(null)}}} className="flex-1 rounded-lg bg-emerald-600/80 px-3 py-2 text-xs font-black text-white disabled:opacity-50"><CheckCircle className="w-3.5 h-3.5 inline mr-1"/>รับเทรด</button>}
                  <button type="button" disabled={tradeBusyId===trade.id} onClick={async()=>{setTradeBusyId(trade.id);try{await onCancelItemTrade?.(trade.id)}catch(e){alert(e instanceof Error?e.message:'ยกเลิกเทรดไม่สำเร็จ')}finally{setTradeBusyId(null)}}} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-black text-slate-300 disabled:opacity-50"><XCircle className="w-3.5 h-3.5 inline mr-1"/>ยกเลิก</button>
                </div>
              </div>;
            })}
          </div>
      ...
      {tradeItem && (<div className="fixed inset-0 z-[86] bg-black/75 flex items-center justify-center p-4">
        <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-slate-900 border border-fuchsia-500/40 p-5">
          <div className="flex items-center gap-2"><ArrowLeftRight className="w-5 h-5 text-fuchsia-300"/><h3 className="text-lg font-black text-white">🔄 สร้างข้อเสนอเทรด</h3></div>
          <p className="text-xs text-slate-400 mt-1">ระบบจะตรวจสอบไอเทมและ Coins อีกครั้งตอนรับเทรด เพื่อป้องกันการเทรดซ้ำ</p>
          <div className="mt-4 rounded-2xl border border-fuchsia-500/30 bg-fuchsia-950/10 p-4">
            <div className="text-xs font-black text-fuchsia-200">สิ่งที่คุณเสนอ</div><div className="text-sm font-black text-white mt-1">{tradeItem.name}</div>
            <div className="grid grid-cols-2 gap-2 mt-3"><label className="text-[11px] text-slate-400">จำนวน<input type="number" min="1" max={Math.max(1,Number(tradeItem.quantity)||1)} value={tradeQuantity} onChange={e=>setTradeQuantity(e.target.value)} className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2 text-white"/></label><label className="text-[11px] text-slate-400">Coins เพิ่ม<input type="number" min="0" max={Math.max(0,Number(character.coins)||0)} value={tradeCoins} onChange={e=>setTradeCoins(e.target.value)} className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2 text-white"/></label></div>
          </div>
          <label className="block text-[11px] text-slate-400 mt-4">ผู้เล่นที่ต้องการเทรดด้วย<select value={tradeTarget} onChange={e=>{setTradeTarget(e.target.value);setTradeRequestedItemId('');setTradeRequestedQuantity('1')}} className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-3 text-white"><option value="">เลือกผู้เล่น...</option>{allCharacters.filter(x=>x.id!==character.id).map(x=><option key={x.id} value={x.id}>{x.displayName}</option>)}</select></label>
          {tradeTarget && (() => { const target=allCharacters.find(x=>x.id===tradeTarget); const targetInventory=target?.inventory||[]; return <div className="mt-4 rounded-2xl border border-cyan-500/30 bg-cyan-950/10 p-4"><div className="text-xs font-black text-cyan-200">สิ่งที่คุณต้องการจากอีกฝ่าย</div><select value={tradeRequestedItemId} onChange={e=>setTradeRequestedItemId(e.target.value)} className="mt-2 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-3 text-white"><option value="">ไม่ขอไอเทม</option>{targetInventory.filter(x=>!x.isEquipped).map(x=><option key={x.instanceId} value={x.instanceId}>{x.name} ×{Math.max(1,Number(x.quantity)||1)}</option>)}</select>{tradeRequestedItemId && <label className="block text-[11px] text-slate-400 mt-2">จำนวนที่ขอ<input type="number" min="1" max={Math.max(1,Number(targetInventory.find(x=>x.instanceId===tradeRequestedItemId)?.quantity)||1)} value={tradeRequestedQuantity} onChange={e=>setTradeRequestedQuantity(e.target.value)} className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2 text-white"/></label>}<label className="block text-[11px] text-slate-400 mt-3">Coins ที่ขอจากอีกฝ่าย<input type="number" min="0" value={tradeRequestedCoins} onChange={e=>setTradeRequestedCoins(e.target.value)} className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2 text-white"/></label></div>; })()}
          <div className="flex gap-2 mt-5"><button type="button" onClick={()=>setTradeItem(null)} className="flex-1 rounded-xl bg-slate-800 py-3 text-xs font-black text-slate-300">ยกเลิก</button><button type="button" disabled={!tradeTarget || !onCreateItemTrade} onClick={async()=>{const qty=Math.max(1,Math.min(Math.floor(Number(tradeQuantity)||1),Math.max(1,Number(tradeItem.quantity)||1)));const coins=Math.max(0,Math.min(Math.floor(Number(tradeCoins)||0),Math.max(0,Number(character.coins)||0)));const target=allCharacters.find(x=>x.id===tradeTarget);const req=target?.inventory?.find(x=>x.instanceId===tradeRequestedItemId);const reqQty=tradeRequestedItemId?Math.max(1,Math.min(Math.floor(Number(tradeRequestedQuantity)||1),Math.max(1,Number(req?.quantity)||1))):0;const reqCoins=Math.max(0,Math.floor(Number(tradeRequestedCoins)||0));try{await onCreateItemTrade?.(tradeTarget,tradeItem.instanceId,qty,coins,tradeRequestedItemId||undefined,reqQty,reqCoins);setTradeItem(null);alert('ส่งข้อเสนอเทรดแล้ว')}catch(e){alert(e instanceof Error?e.message:'สร้างข้อเสนอเทรดไม่สำเร็จ')}}} className="flex-1 rounded-xl bg-fuchsia-600 py-3 text-xs font-black text-white disabled:opacity-40">ส่งข้อเสนอเทรด</button></div>
        </div>
      </div>)}
