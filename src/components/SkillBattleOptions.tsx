        ...
        <div className="space-y-1">
          {(u.skills || []).map(s=>{
            ...
            return <div key={s.id} className="col-span-2 rounded-2xl border border-violet-400/20 bg-slate-950/50 p-3 space-y-3">
              ...
              <div className="space-y-3 rounded-2xl border border-violet-400/20 bg-slate-950/40 p-3">
              ...
              <div className="rounded-xl border border-violet-400/20 bg-violet-950/10 p-3 space-y-2">
                <label className="block text-[9px] text-slate-400">🎲 โอกาสใช้สกิล (%)
                  <input type="number" min="0" max="100" value={s.aiChancePercent ?? 100} onChange={e=>updateUnitSkill(u,s.id,{aiChancePercent:Math.max(0,Math.min(100,num(e.target.value,100)))})} className="mt-1 w-full rounded-lg bg-slate-900 border border-violet-300/20 px-2 py-2 text-white" />
                </label>
              </div>
              <div>
                <label className="block text-[9px] text-slate-400">⏱️ คูลดาวน์ (เทิร์น)<input type="number" min="0" value={s.cooldownTurns ?? 0} onChange={e=>updateUnitSkill(u,s.id,{cooldownTurns:Math.max(0,num(e.target.value))})} className="mt-1 w-full rounded-lg bg-slate-900 border border-slate-700 px-2 py-2 text-white"/></label>
              </div>
            </div>
              </div>
          </div>
