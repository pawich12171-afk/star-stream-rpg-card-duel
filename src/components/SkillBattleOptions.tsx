  ...
  return <div className="space-y-3 rounded-xl border border-violet-500/25 bg-violet-950/10 p-3">
    {showTarget && <label className="block text-[10px] text-slate-400">
      ...
      <select value={config.battleEffect || 'damage'} onChange={e=>{
        ...
        <option value="damage_taken_increase">💥 รับดาเมจมากขึ้น (%)</option>
    ...
    <div className="rounded-xl border border-fuchsia-400/20 bg-fuchsia-950/10 p-3 space-y-2"><div className="flex items-center justify-between gap-2"><div><div className="text-[11px] font-black text-fuchsia-100">✨/⚠️ บัฟและดีบัฟหลายรายการ</div><div className="text-[9px] text-slate-500">กำหนด Stat/สถานะ ค่า ระยะเวลา และโอกาสแยกกันได้</div></div><div className="flex flex-wrap gap-1.5 w-full sm:w-auto"><button type="button" onClick={()=>addModifier('buff')} className="rounded-lg bg-emerald-500/20 px-2 py-1 text-[9px] text-emerald-100">+ บัฟ</button><button type="button" onClick={()=>addModifier('debuff')} className="rounded-lg bg-rose-500/20 px-2 py-1 text-[9px] text-rose-100">+ ดีบัฟ</button></div></div>{modifiers.map(item=><div key={item.id} className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 rounded-lg border border-slate-700 bg-slate-950/60 p-2">
        ...
        {item.kind === 'status' && <label className="text-[8px] text-slate-500">💫 สถานะ
          <select value={item.status || ''} onChange={e=>updateModifier(item.id,{status:e.target.value})} className="mt-0.5 w-full rounded bg-slate-900 border border-slate-700 px-1.5 py-1 text-[9px] text-white"><option value="">เลือกสถานะ</option><option value="stun">💫 Stun — ข้ามเทิร์น</option><option value="freeze">❄️ Freeze — หยุดการกระทำ</option><option value="poison">☠️ Poison — ดาเมจต่อเทิร์น</option><option value="burn">🔥 Burn — ดาเมจต่อเทิร์น</option><option value="bleeding">🩸 Bleeding — ดาเมจต่อเทิร์น</option><option value="slow">🐌 Slow — ลดประสิทธิภาพโจมตี</option><option value="weakness">📉 Weakness — ลดดาเมจ</option><option value="curse">🌀 Curse — ลดดาเมจ</option><option value="regen">💚 Regen — ฟื้น HP ต่อเทิร์น</option><option value="reduce_max_hp_percent">❤️‍🩹 ลด Max HP (%)</option><option value="reduce_defense_percent">🛡️ ลดป้องกัน (%)</option><option value="damage_percent">⚔️ เพิ่ม/ลดดาเมจ (%)</option><option value="heal_percent">💚 ฟื้น HP (%)</option><option value="shield">🛡️ โล่ HP</option><option value="reflect">🔄 สะท้อนดาเมจ (%)</option><option value="damage_reduction">🛡️ ลดความเสียหาย (%)</option><option value="damage_taken_increase">💥 รับดาเมจมากขึ้น (%)</option></select>
        </label>}
        ...
        <label className="text-[8px] text-slate-500"><span>{item.kind === 'shield' ? '🛡️ ปริมาณโล่ (HP)' : item.kind === 'buff' || item.kind === 'debuff' ? '📊 ค่าเพิ่ม/ลด Stat' : item.kind === 'cleanse' ? '🧼 จำนวนสถานะที่ล้าง' : ['reduce_max_hp_percent','reduce_defense_percent','damage_percent','heal_percent','reflect','damage_reduction','damage_taken_increase'].includes(String(item.status)) ? '🎯 เปอร์เซ็นต์ผล (%)' : '💫 ค่าผลสถานะ'}</span><input type="number" min="0" max={['reduce_max_hp_percent','reduce_defense_percent','damage_percent','heal_percent','reflect','damage_reduction'].includes(String(item.status)) ? 100 : ['damage_taken_increase'].includes(String(item.status)) ? 1000 : undefined} value={item.value ?? 0} onChange={e=>{const raw=num(e.target.value); const percent=['reduce_max_hp_percent','reduce_defense_percent','damage_percent','heal_percent','reflect','damage_reduction','damage_taken_increase'].includes(String(item.status)); const maxPercent=String(item.status)==='damage_taken_increase'?1000:100; updateModifier(item.id,{value:percent ? Math.max(0,Math.min(maxPercent,raw)) : Math.max(0,raw)})}} placeholder={item.kind === 'shield' ? 'เช่น 100 HP' : ['reduce_max_hp_percent','reduce_defense_percent','damage_percent','heal_percent','reflect','damage_reduction','damage_taken_increase'].includes(String(item.status)) ? 'เช่น 20%' : 'เช่น 10'} className="mt-0.5 w-full rounded bg-slate-900 border border-slate-700 px-1.5 py-1 text-[9px] text-white"/></label>
