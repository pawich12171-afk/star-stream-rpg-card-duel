      ...
      {activeTab === 'admin_items' && (
        <div className="space-y-5">
          <div className="rounded-3xl border border-fuchsia-500/30 bg-fuchsia-950/10 p-5">
            <div className="mb-4 flex items-center gap-3"><Package className="h-6 w-6 text-fuchsia-300" /><div><h3 className="text-lg font-black text-white">จัดการไอเทมทั้งหมด</h3><p className="text-xs text-fuchsia-200/70">ไอเทมทุกชิ้นในระบบใช้ร่วมกันทั้งคลังผู้เล่น กาชา และร้านค้า</p></div></div>
            <form onSubmit={async (e) => {
              ...
              <label className="text-xs text-slate-400">ราคา<input className={inputClass+' mt-1'} type="number" min="0" value={shopItemPrice} onChange={e=>setShopItemPrice(Number(e.target.value))}/></label>
              <label className="text-xs text-slate-400">ประเภท<select className={inputClass+' mt-1'} value={shopItemCategory} onChange={e=>setShopItemCategory(e.target.value as any)}><option value="consumable">ของใช้</option><option value="equipment">อุปกรณ์</option></select></label>
              <label className="text-xs text-slate-400">ความหายาก<select className={inputClass+' mt-1'} value={shopItemRarity} onChange={e=>setShopItemRarity(e.target.value as GachaRarity)}><option value="common">Common</option><option value="rare">Rare</option><option value="epic">Epic</option><option value="legendary">Legendary</option><option value="mythic">Mythic</option></select></label>
              <label className="text-xs text-slate-400">ผลหลัก<select className={inputClass+' mt-1'} value={shopItemEffectType} onChange={e=>setShopItemEffectType(e.target.value as any)}><option value="heal_hp">ฟื้น HP</option><option value="boost_max_hp">เพิ่ม Max HP</option><option value="buff_stat">เพิ่มสเตตัส</option><option value="enhance_skill">เสริมสกิล</option><option value="custom">กำหนดเอง</option></select></label>
              {shopItemEffectType==='buff_stat' && <label className="text-xs text-slate-400">สเตตัส<select className={inputClass+' mt-1'} value={shopItemTargetStat} onChange={e=>setShopItemTargetStat(e.target.value as any)}><option value="strength">STR</option><option value="durability">DUR</option><option value="agility">AGI</option><option value="magic">MAG</option></select></label>}
              <label className="text-xs text-slate-400">ค่าผล<input className={inputClass+' mt-1'} type="number" value={shopItemEffectVal} onChange={e=>setShopItemEffectVal(Number(e.target.value))}/></label>
