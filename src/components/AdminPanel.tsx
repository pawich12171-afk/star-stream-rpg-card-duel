...
export const AdminPanel: React.FC<AdminPanelProps> = ({
                  ...
                  <div className="space-y-2 rounded-xl border border-cyan-500/20 bg-cyan-950/20 p-3">
                    ...
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                      <select value={newRewardBattleEffect} onChange={(e) => setNewRewardBattleEffect(e.target.value as NonNullable<Skill['battleEffect']>)} className="w-full rounded-lg border border-slate-700 bg-slate-800 px-2 py-2 text-xs text-white outline-none">
                        <option value="damage">โจมตี / ดาเมจ</option><option value="heal">ฟื้นฟู HP</option><option value="defense">โล่ / ป้องกัน</option><option value="reflect">สะท้อนดาเมจ</option><option value="stun">ควบคุม / สตัน</option><option value="copy_ability">🧬 คัดลอกความสามารถศัตรู</option><option value="immortal">♾️ อมตะ</option><option value="damage_reduction">🛡️ ลดความเสียหาย</option><option value="damage_taken_increase">💥 รับดาเมจมากขึ้น (%)</option><option value="buff_stat">💪 บัฟสเตตัส</option><option value="summon">🧿 เสกลูกน้อง</option>
                      </select>
  ...
  onChange={(patch) => {
    if(patch.battleEffect) setNewRewardBattleEffect(patch.battleEffect);
                    ...
                    <div className="rounded-lg border border-fuchsia-500/20 bg-fuchsia-950/10 p-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
                        <select value={newRewardEffectKind} onChange={e => setNewRewardEffectKind(e.target.value as BattleExtraEffect['kind'])} className="rounded-lg border border-slate-700 bg-slate-800 px-2 py-2 text-xs text-white">
                          <option value="bleeding">เลือดไหล</option><option value="burn">เผาไหม้</option><option value="poison">พิษ</option><option value="freeze">Freeze</option><option value="stun">สตัน</option><option value="reduce_max_hp_percent">ลด MAX HP %</option><option value="reduce_defense_percent">ลดป้องกัน %</option><option value="damage_percent">เพิ่มดาเมจ %</option><option value="damage_taken_increase">💥 รับดาเมจมากขึ้น %</option><option value="heal_percent">ฟื้น HP %</option><option value="shield">โล่</option><option value="reflect">สะท้อน %</option>
                        </select>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-cyan-500/40 bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <select value={editingSkillEffect} onChange={e => setEditingSkillEffect(e.target.value as NonNullable<Skill['battleEffect']>)} className="rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-white"><option value="damage">โจมตี / ดาเมจ</option><option value="heal">ฟื้นฟู HP</option><option value="defense">ป้องกัน</option><option value="reflect">สะท้อนดาเมจ (%)</option><option value="reflect_no_damage">สะท้อนดาเมจ + ไม่รับดาเมจ</option><option value="stun">สตัน</option><option value="copy_ability">🧬 คัดลอกความสามารถศัตรู</option><option value="immortal">♾️ อมตะ / กัน True Damage</option><option value="damage_reduction">🛡️ ลดความเสียหาย</option><option value="damage_taken_increase">💥 รับดาเมจมากขึ้น (%)</option><option value="buff_stat">💪 บัฟสเตตัส</option><option value="summon">🧿 เสกลูกน้อง</option></select>
            ...
            <SkillBattleOptions
              ...
              onChange={(patch) => {
                if(patch.battleEffect) setEditingSkillEffect(patch.battleEffect);
            ...
            />              <label className="text-[10px] text-slate-400">{editingSkillEffect === 'reflect' || editingSkillEffect === 'reflect_no_damage' ? '🔄 สะท้อนดาเมจ (%)' : editingSkillEffect === 'damage_reduction' ? '📉 ลดดาเมจ (%)' : editingSkillEffect === 'defense' ? '🛡️ ป้องกัน (% Max HP)' : editingSkillEffect === 'heal' ? '❤️ ฟื้นฟู (% Max HP)' : editingSkillEffect === 'damage' ? '⚔️ ดาเมจ' : 'พลังสกิล'}<input type="number" min={1} value={editingSkillPower} onChange={e => setEditingSkillPower(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-white" /></label>
              <label className="text-[10px] text-slate-400">คูลดาวน์
                <input type="number" min={0} value={editingSkillCooldown} onChange={e => setEditingSkillCooldown(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-white" />
              </label>
              {['reflect','reflect_no_damage','damage_reduction','defense','heal'].includes(String(editingSkillEffect)) && <label className="text-[10px] text-slate-400">⏱️ ระยะเวลาคงอยู่ (เทิร์น)
                <input type="number" min={1} max={10} value={editingSkillEffectDuration} onChange={e => setEditingSkillEffectDuration(Math.max(1, Math.min(10, Number(e.target.value)||1)))} className="mt-1 w-full rounded-xl border border-amber-400/40 bg-slate-800 px-3 py-2 text-xs text-white" />
              </label>}
